...
async function enterPaymentDetails(page){
  ...
  let iframeExpiration = await page.waitForSelector("iframe[title='Field container for: Expiration date (MM / YY)']");
  let iframeExpirationContent = await iframeExpiration.contentFrame();
  await iframeExpirationContent.type("input[id='expiry']", "10 / 25");

  let iframeCode = await page.waitForSelector(("iframe[title='Field container for: Security code']");
